...
router.get("/summary", verifyFirebaseToken, async (req, res) => {
    try {
      ...
      `, [dbUserId]);
        ...
        const pendingIncoming = adjustedSettlementResult.rows.reduce((total, row) => row.direction === "incoming" ? total + Number(row.amount) : total, 0);
        const pendingOutgoing = adjustedSettlementResult.rows.reduce((total, row) => row.direction === "outgoing" ? total + Number(row.amount) : total, 0);
        return res.json({
            summary: {
                availableBalance,
                pendingIncoming,
