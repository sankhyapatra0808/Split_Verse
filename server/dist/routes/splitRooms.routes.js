...
import { moneyAmountSchema, parseRequestBody, safeTextSchema, sendValidationError, } from "../middleware/validateRequest.js";
...
const reminderActionSchema = z
    ...
    .strict();
let splitRoomTablesReady = null;
let netSettlementTablesReady = null;
...
function roundMoneyValue(amount) {
    return Math.round((amount + Number.EPSILON) * 100) / 100;
}
function getPersonLabel(name, email) {
    return name?.trim() || email.split("@")[0] || "Member";
}
function getDebtLineQuery({ currentUserOnly = false, pairOnly = false, forUpdate = false, } = {}) {
    const filters = [
    ...
    if (currentUserOnly) {
        filters.push("(COALESCE(room.paid_by_user_id, room.owner_user_id) = $1 OR COALESCE(assigned_user.id, member.user_id) = $1)");
    }
    if (pairOnly) {
        filters.push(`
      (
...
async function loadNetDebtLines(client, currentUserId) {
    const result = await client.query(getDebtLineQuery({ currentUserOnly: true }), [currentUserId]);
...
async function loadPairDebtLines(client, leftUserId, rightUserId, { forUpdate = false } = {}) {
    const result = await client.query(getDebtLineQuery({ pairOnly: true, forUpdate }), [leftUserId, rightUserId]);
    return result.rows.map((line) => ({
        ...line,
        original_amount: Number(line.original_amount),
        settled_amount: Number(line.settled_amount),
...
function serializeNetSettlements(debtLines, currentUserId) {
...
async function insertItemSettlement(client, { itemId, amount, method, createdByUserId, counterItemId = null, walletTransactionId = null, }) {
    await client.query(`
...
async function refreshRoomPaymentStatuses(client, roomIds) {
...
async function applyOffsetSettlements(client, forwardLines, reverseLines, amountToOffset, createdByUserId) {
...
function memberToIdentity(member) {
...
async function findUsersByEmail(client, emails) {
    if (emails.length === 0) {
        return new Map();
    }
    const result = await client.query(`
    ...
    `, [emails]);
    return new Map(result.rows.map((user) => [user.email.toLowerCase(), user]));
}
async function insertSplitRoomMembers(client, rows) {
    for (const chunk of chunkArray(rows, splitRoomMemberInsertChunkSize)) {
        const values = [];
        const placeholders = chunk
            .map((row, rowIndex) => {
            const paramIndex = rowIndex * 6;
            values.push(row.roomId, row.userId, row.displayName, row.email, row.role, row.status);
...
function serializeRoom(room, members, items, currentUser) {
    ...
    const serializedMembers = members.map((member) => {
        ...
        return {
            ...
            display_photo_url: avatarMode === "initials"
                ? null
    ...
    const memberById = new Map(serializedMembers.map((member) => [member.id, member]));
    items.forEach((item) => {
        ...
        itemCountByMember.set(item.assigned_member_id, (itemCountByMember.get(item.assigned_member_id) ?? 0) + 1);
        amountByMember.set(item.assigned_member_id, (amountByMember.get(item.assigned_member_id) ?? 0) + amount);
        if (!member?.isMe) {
            if (settledAmount > 0) {
    ...
    const outstandingAmount = Array.from(outstandingByMember.values()).reduce((sum, amount) => sum + amount, 0);
    const collectedAmount = Array.from(collectedByMember.values()).reduce((sum, amount) => sum + amount, 0);
    const paymentStatus = normalizePaymentStatus(room.payment_status);
    const statusLabel = paymentStatus === "complete"
        ? "Complete"
...
function getRouteParam(req, paramName) {
...
router.get("/", verifyFirebaseToken, async (req, res) => {
    try {
      ...
      `, [dbUser.id, dbUser.email]);
        ...
        const rooms = roomResult.rows.map((room) => serializeRoom(room, memberResult.rows.filter((member) => member.room_id === room.id), itemResult.rows.filter((item) => item.room_id === room.id), dbUser));
        return res.json({ rooms });
    }
    catch (error) {
...
router.get("/net-settlements", verifyFirebaseToken, async (req, res) => {
    try {
        await ensureSplitRoomTablesOnce();
        await ensureNetSettlementTablesOnce();
        const firebaseUser = req.user;
...
router.post("/net-settlements/pay", verifyFirebaseToken, async (req, res) => {
    const client = await db.connect();
    try {
        await ensureSplitRoomTablesOnce();
        await ensureNetSettlementTablesOnce();
        ...
        const { toUserId, walletPin } = parseRequestBody(netSettlementPaymentSchema, req.body);
        await client.query("BEGIN");
        ...
        if (offsetAmount > 0) {
            touchedItemIds.push(...(await applyOffsetSettlements(client, payerOwesReceiver, receiverOwesPayer, offsetAmount, payer.id)));
        }
        ...
        const debitWalletTransaction = walletTransactionResult.rows.find((row) => row.user_id === payer.id && row.type === "debit");
        ...
        touchedItemIds.push(...(await applyWalletSettlement(client, payerOwesReceiver, netAmount, payer.id, debitWalletTransaction.id)));
        const collectedItems = await markFullySettledItemsCollected(client, touchedItemIds);
        await refreshRoomPaymentStatuses(client, collectedItems.map((item) => item.room_id));
        await client.query("COMMIT");
...
router.post("/", verifyFirebaseToken, async (req, res) => {
    ...
    try {
      ...
      `, [dbUser.id]);
        const roomsCreatedToday = Number(roomsCreatedTodayResult.rows[0].room_count);
        ...
        const paidByUser = normalizedPaidByEmail === dbUser.email.toLowerCase()
            ? dbUser
...
router.post("/:roomId/items", verifyFirebaseToken, async (req, res) => {
    try {
        await ensureSplitRoomTablesOnce();
        await ensureNetSettlementTablesOnce();
        const firebaseUser = req.user;
        ...
        const { title, assignedMemberId, amount } = parseRequestBody(createSplitRoomItemSchema, req.body);
        if (!roomId) {
      ...
      `, [roomId, dbUser.id]);
        ...
        const assignedMemberResult = await db.query(`
...
router.post("/:roomId/members/:memberId/collect", verifyFirebaseToken, async (req, res) => {
    const client = await db.connect();
    try {
        await ensureSplitRoomTablesOnce();
        await ensureNetSettlementTablesOnce();
...
router.patch("/items/:itemId", verifyFirebaseToken, async (req, res) => {
    ...
    try {
        ...
        const { title, amount } = parseRequestBody(updateSplitRoomItemSchema, req.body);
        if (!itemId) {
...
router.delete("/:roomId/members/:memberId", verifyFirebaseToken, async (req, res) => {
    const client = await db.connect();
    try {
        await ensureSplitRoomTablesOnce();
        const firebaseUser = req.user;
        ...
        `, [roomId, memberId]);
        const assignedItemCount = Number(assignedItemsResult.rows[0]?.item_count ?? 0);
        if (assignedItemCount > 0) {
            await client.query("ROLLBACK");
            return res.status(409).json({
                message: "This member has room history, so removing them would affect previous split records.",
            });
        }
        ...
        AND user_id IS NOT NULL;
        `, [roomId]);
        const notifiedUserIds = notifiedUserIdsResult.rows.map((row) => row.user_id);
        await client.query(`
...
router.patch("/:roomId/payment-status", verifyFirebaseToken, async (req, res) => {
    const client = await db.connect();
    try {
        await ensureSplitRoomTablesOnce();
        const firebaseUser = req.user;
...
router.post("/:roomId/reminders", verifyFirebaseToken, async (req, res) => {
    try {
        await ensureSplitRoomTablesOnce();
        await ensureNetSettlementTablesOnce();
        const firebaseUser = req.user;
...
router.post("/:roomId/reminders/mute", verifyFirebaseToken, async (req, res) => {
    try {
        await ensureSplitRoomTablesOnce();
        await ensureNetSettlementTablesOnce();
        const firebaseUser = req.user;
...
router.post("/:roomId/reminders/discussed", verifyFirebaseToken, async (req, res) => {
    try {
        await ensureSplitRoomTablesOnce();
        await ensureNetSettlementTablesOnce();
        const firebaseUser = req.user;
...
router.post("/:roomId/finalize", verifyFirebaseToken, async (req, res) => {
    const client = await db.connect();
    try {
        await ensureSplitRoomTablesOnce();
        await ensureNetSettlementTablesOnce();
...
router.post("/:roomId/archive", verifyFirebaseToken, async (req, res) => {
    try {
        await ensureSplitRoomTablesOnce();
        await ensureNetSettlementTablesOnce();
        const firebaseUser = req.user;
...
router.post("/items/:itemId/pay", verifyFirebaseToken, async (req, res) => {
    const client = await db.connect();
    try {
        await ensureSplitRoomTablesOnce();
        await ensureNetSettlementTablesOnce();
        ...
        if (item.assigned_user_id !== payerUserId &&
        ...
        const debitWalletTransaction = walletTransactionResult.rows.find((row) => row.user_id === payerUserId && row.type === "debit");
        const expenseResult = await client.query(`
        INSERT INTO expenses (
          user_id,
          title,
...
router.get("/pending-dues", verifyFirebaseToken, async (req, res) => {
    try {
        await ensureSplitRoomTablesOnce();
        await ensureNetSettlementTablesOnce();
        const firebaseUser = req.user;
