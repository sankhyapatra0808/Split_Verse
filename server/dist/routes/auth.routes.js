...
import { requireRecentAuthentication, verifyFirebaseCredentialToken, verifyFirebaseToken, } from "../middleware/verifyFirebaseToken.js";
...
const loginOtpExpiryMs = Number(process.env.LOGIN_OTP_EXPIRY_MS || 10 * 60 * 1000);
...
const loginOtpRequestWindowMinutes = Number(process.env.LOGIN_OTP_REQUEST_WINDOW_MINUTES || 15);
const loginOtpMaxResends = Number(process.env.LOGIN_OTP_MAX_RESENDS || 3);
const loginOtpResendCooldownMs = Number(process.env.LOGIN_OTP_RESEND_COOLDOWN_MS || 30 * 1000);
...
const walletPinOtpPepper = process.env.WALLET_PIN_RESET_OTP_PEPPER?.trim() ||
...
const passwordResetOtpPepper = process.env.PASSWORD_RESET_OTP_PEPPER?.trim() ||
    (process.env.NODE_ENV === "production"
        ? ""
...
const walletPinSchema = z
    ...
    username: z
        ...
        .refine((value) => /^[a-z0-9_]{3,30}$/.test(value), "Username must be 3 to 30 characters using lowercase letters, numbers, or underscores")
        .optional(),
})
    .strict();
...
function assertOtpPepperConfigured(secret, feature) {
    const sufficientlyStrong = secret.length >= 32;
    if (sufficientlyStrong || process.env.NODE_ENV !== "production")
        return;
...
function hashLoginOtp(otp, sessionId, firebaseUid) {
...
async function resolveLoginEmail(identifier) {
    ...
    const looksLikeUsername = normalizedIdentifier.startsWith("@") ||
...
function createProfileIdentityOtp() {
    return crypto
        .randomInt(10 ** (profileIdentityOtpLength - 1), 10 ** profileIdentityOtpLength)
        .toString();
}
function hashProfileIdentityOtp(otp, requestId, userId, changeType) {
    const pepper = loginOtpPepper || "splitverse-development-login-otp-pepper";
    return crypto
        .createHmac("sha256", pepper)
...
async function sendProfileIdentityOtpEmail({ currentEmail, otp, changeType, }) {
    if (!isEmailConfigured()) {
        return "not_configured";
    }
...
async function generateAvailableUsername(preferredBase, excludeUserId = null) {
...
function normalizeProfilePhotoUrl(value) {
    ...
    try {
        ...
        const httpAllowed = process.env.NODE_ENV !== "production" && parsedUrl.protocol === "http:";
...
router.post("/username/availability", async (req, res) => {
    try {
        const { username } = parseRequestBody(usernameAvailabilitySchema, req.body);
        const result = await db.query(`
      SELECT 1
      FROM users
      WHERE LOWER(username) = LOWER($1)
...
router.post("/email-login-otp/request", async (req, res) => {
    try {
        ...
        const { identifier, password } = parseRequestBody(loginOtpRequestSchema, req.body);
      ...
      `, [credential.uid, loginOtpRequestWindowMinutes]);
        if (Number(requestCountResult.rows[0]?.request_count || 0) >=
            loginOtpMaxRequests) {
        ...
        const code = typeof error === "object" && error !== null && "code" in error
            ? String(error.code || "LOGIN_OTP_REQUEST_FAILED")
    ...
    try {
      ...
      `, [sessionId]);
        ...
        if (new Date(session.expires_at).getTime() <= Date.now()) {
            await client.query(`UPDATE email_login_otp_sessions SET consumed_at = NOW() WHERE id = $1;`, [session.id]);
            await client.query("COMMIT");
        ...
        const elapsedSinceLastSend = Date.now() - new Date(session.last_sent_at).getTime();
        if (elapsedSinceLastSend < loginOtpResendCooldownMs) {
            ...
            return res.status(429).json({
                ...
                retryAfterSeconds: Math.ceil((loginOtpResendCooldownMs - elapsedSinceLastSend) / 1000),
            });
        }
        const otp = createLoginOtp();
...
router.post("/email-login-otp/verify", async (req, res) => {
    ...
    try {
      ...
      `, [sessionId]);
        ...
        const customToken = await adminAuth.createCustomToken(session.firebase_uid, {
...
router.post("/sync-user", verifyFirebaseCredentialToken, async (req, res) => {
    try {
        const { username: requestedUsername, requireUsername, deferUsernameSetup, } = parseRequestBody(syncUserSchema, req.body ?? {});
        ...
        const existingUserResult = await db.query(`SELECT id, username FROM users WHERE firebase_uid = $1 LIMIT 1;`, [firebaseUid]);
        const existingUser = existingUserResult.rows[0];
        if (!existingUser?.username && !requestedUsername && requireUsername) {
            return res.status(400).json({
                code: "USERNAME_REQUIRED",
                message: "Choose your permanent username before creating your account.",
            });
        }
        const canDeferUsernameSetup = deferUsernameSetup &&
            ["google", "google.com"].includes(String(provider || ""));
        const username = existingUser?.username ||
            requestedUsername ||
            (canDeferUsernameSetup
                ? null
                : await generateAvailableUsername(buildUsernameBase(name, email), existingUser?.id ?? null));
        const result = await db.query(`
      INSERT INTO users (
        firebase_uid,
        name,
...
router.post("/profile/identity-change/request", verifyFirebaseToken, async (req, res) => {
    try {
        assertLoginOtpConfigured();
        const firebaseUser = req.user;
        if (!firebaseUser) {
        ...
        const { field, value } = parseRequestBody(profileIdentityChangeRequestSchema, req.body);
        ...
        if (field === "name") {
            ...
            const emailInUse = await db.query(`SELECT 1 FROM users WHERE LOWER(email) = LOWER($1) LIMIT 1;`, [pendingValue]);
...
router.post("/profile/identity-change/confirm", verifyFirebaseToken, async (req, res) => {
    const client = await db.connect();
    let firebaseUidForRollback = "";
    let firebaseRollback = null;
    try {
        assertLoginOtpConfigured();
        const firebaseUser = req.user;
        if (!firebaseUser) {
        ...
        const { requestId, otp } = parseRequestBody(profileIdentityChangeConfirmSchema, req.body);
        await client.query("BEGIN");
        ...
        if (new Date(changeRequest.expires_at).getTime() <= Date.now()) {
            await client.query(`UPDATE profile_identity_change_otps SET consumed_at = NOW() WHERE id = $1;`, [requestId]);
            await client.query("COMMIT");
            return res.status(410).json({
        ...
        const expectedHash = hashProfileIdentityOtp(otp, requestId, changeRequest.user_id, changeRequest.change_type);
        if (!timingSafeEqualHex(expectedHash, changeRequest.otp_hash)) {
          ...
          `, [requestId, nextAttempts, profileIdentityOtpMaxAttempts]);
            await client.query("COMMIT");
            return res.status(401).json({
                message: nextAttempts >= profileIdentityOtpMaxAttempts
        ...
        return res.json({
            message: changeRequest.change_type === "name"
    ...
    try {
        ...
        if (usernameSupplied) {
            return res.status(403).json({
                ...
                message: "Your SplitVerse username is permanent and cannot be changed after signup.",
            });
        }
...
router.post("/wallet-pin", verifyFirebaseToken, async (req, res) => {
    ...
    try {
        ...
        const { pin, currentPin, username: requestedUsername } = parseRequestBody(walletPinSchema, req.body);
        await client.query("BEGIN");
      ...
      `, [firebaseUser.uid]);
        ...
        const isGoogleUser = ["google", "google.com"].includes(String(user.provider || ""));
...
router.post("/wallet-pin/reset-otp/request", verifyFirebaseToken, async (req, res) => {
    ...
    try {
        ...
        if (Number(requestCountResult.rows[0].request_count) >=
...
router.delete("/account", verifyFirebaseToken, requireRecentAuthentication(), async (req, res) => {
    const client = await db.connect();
    try {
        const firebaseUser = req.user;
        if (!firebaseUser) {
