...
router.get("/summary", verifyFirebaseToken, async (req, res) => {
    try {
      ...
      AND split_room_items.collected_at IS NULL;
      `, [dbUserId, dbUserEmail]);
        const pendingSplitPayment = Number(pendingSplitDuesResult.rows[0].pending_payment);
        const receivableSplitAmount = Number(receivableSplitDuesResult.rows[0].receivable);
