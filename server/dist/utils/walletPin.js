...
export function normalizeWalletPin(value) {
    return String(value ?? "").replace(/\D/g, "");
}
export function assertWalletPinShape(pin) {
    if (!/^\d{4,6}$/.test(pin)) {
        throw new WalletPinError("Enter your 4 to 6 digit wallet PIN", 400);
    }
}
export async function verifyWalletPinForUser(client, userId, rawPin) {
    ...
    if (lockedUntil > Date.now()) {
        throw new WalletPinError("Wallet PIN is temporarily locked. Try again later.", 423, { lockedUntil: new Date(lockedUntil).toISOString() });
    }
    ...
    if (!matches) {
      ...
      `, [user.id, nextAttempts, lockedUntilValue]);
        throw new WalletPinError(shouldLock
            ? "Too many wrong wallet PIN attempts. Wallet PIN is temporarily locked."
            : "Incorrect wallet PIN", shouldLock ? 423 : 401, {
...
export function sendWalletPinError(res, error) {
