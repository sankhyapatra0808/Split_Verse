...
export default function MandatoryWalletPinSetup() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      setSaving(true);
      await withTopProgress(() => saveWalletPin({ pin }));
      await refreshDbUser();
    } catch (submitError) {
      setError(
  ...
  return (
    <div className="mandatory-wallet-pin-screen">
      <form className="mandatory-wallet-pin-card" onSubmit={handleSubmit}>
        ...
        <h1>Set your wallet PIN</h1>
        <p>
          Before opening SplitVerse, create a private 4 to 6 digit PIN. This PIN
          protects wallet payments and split-room dues.
        </p>
        ...
        <button type="submit" disabled={saving}>
          {saving ? "Setting PIN" : "Set wallet PIN"}
        </button>

        <small>
          This window cannot be closed until the PIN is set. Your PIN is stored
